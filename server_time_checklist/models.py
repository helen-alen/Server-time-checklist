...
class ServerTimeChecklist(models.Model):
    ...
    def __str__(self):
        return f"Id: {self.id} ,Asset IP: {self.asset_ip}, Server Name: {self.server_name}, Time Synced: {self.time_synced}, Check Time: {self.check_time}, Is Adjusted: {self.is_adjusted},Checked By: {self.checked_by}"

    # , ReviewTime: {self.review_time}